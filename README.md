...
*   `start_shell`: Starts a new background process. (Label is optional)
...
*   `check_shell`: Checks the status and logs of a specific process. (Requires label)
...
Response payload for a successful start_shell call. Contains fields like `label`, `status`, `pid`, `command`, `args`, `cwd`, `exitCode`, `signal`, `log_file_path`, `tail_command`, `message`, `logs`, `monitoring_hint`, `info_message`.
