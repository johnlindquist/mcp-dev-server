...
import { DEFAULT_RETURN_LOG_LINES, MAX_STORED_LOG_LINES, MAX_RETRIES, STOP_WAIT_DURATION } from './constants.js'; // Added STOP_WAIT_DURATION
...
async function _checkDevServerStatus(label: string, logLines: number): Promise<CallToolResult> {
...
async function _stopAllDevServers(force: boolean): Promise<CallToolResult> {
    ...
    for (const [label, serverInfo] of runningServers.entries()) {
        ...
        const isActive = (serverInfo.status === 'running' || serverInfo.status === 'starting' || serverInfo.status === 'verifying' || serverInfo.status === 'stopping' || serverInfo.status === 'restarting');
        ...
        if (isActive && serverInfo.process && serverInfo.pid) {
            ...
            addLogEntry(label, 'warn', `Server ${label} is active (${serverInfo.status}) but process handle/PID missing. Cannot stop.`);
            signalError = "Process handle or PID missing for active server.";
            updateServerStatus(label, 'error', { error: signalError });
        }
        results.push({ label, pid: serverInfo.pid, initialStatus: serverInfo.status, signalSent: sentSignal, error: signalError });
    }
    ...
    const summary = results.reduce((acc, r) => {
        ...
        else if (status === 'running' || status === 'stopping' || status === 'restarting' || status === 'verifying' || status === 'starting') acc.stillActive++;
        else acc.other++;
        return acc;
    ...
    const payload = {
        ...
        details: results.map(r => ({
            label: r.label, pid: r.pid, initialStatus: r.initialStatus,
            signalSent: r.signalSent, finalStatus: r.finalStatus, error: r.error
        })),
    };
    const result = {
        content: [textPayload(JSON.stringify(payload, null, 2))],
...
async function _restartDevServer(label: string, force: boolean, logLines: number): Promise<CallToolResult> {
    ...
    let stopStatus: ServerStatus | 'not_found' | 'already_stopped' | 'error' = 'not_found'; // Add error possibility
    ...
    if (serverInfo) {
        ...
        if (serverInfo.status === 'running' || serverInfo.status === 'starting' || serverInfo.status === 'verifying' || serverInfo.status === 'stopping' || serverInfo.status === 'restarting' || serverInfo.status === 'crashed') {
            ...
            try {
                const responseText = stopResponse.content[0]?.type === 'text' ? stopResponse.content[0].text : null;
                if (!responseText) throw new Error("Stop response content is missing or not text");
                ...
                if (stopResponse.isError && !stopErrorMsg) {
                    stopErrorMsg = stopResultJson?.message || "Unknown stop error during restart";
                }
                ...
                if (stopStatus === 'stopped' || stopStatus === 'crashed' || stopStatus === 'error') {
                    ...
                    stopErrorMsg = stopErrorMsg || `Server status is still '${stopStatus}' after stop attempt. Cannot proceed with restart.`;
    ...
    if (canStart) {
        ...
        if (startResult.isError) {
            const responseText = startResult.content[0]?.type === 'text' ? startResult.content[0].text : null;
            try {
                startErrorMsg = responseText ? (JSON.parse(responseText) as { error?: string }).error ?? "Unknown start error" : "Unknown start error";
            } catch { startErrorMsg = "Failed to parse start error response."; }
    ...
    return fail(textPayload(JSON.stringify({
        ...
        stopPhase: {
            status: stopStatus,
            error: stopErrorMsg,
            hint: stopStatus === 'error' ? "check_stop_logs" : (stopStatus !== 'stopped' && stopStatus !== 'crashed' && stopStatus !== 'already_stopped') ? "retry_stop_with_force" : "stop_phase_ok"
        },
        startPhase: {
            attempted: canStart,
            error: startErrorMsg,
...
async function _waitForDevServer(label: string, timeoutSeconds: number, checkIntervalSeconds: number, targetStatus: string, logLines: number): Promise<CallToolResult> {
    ...
    while (!achievedTargetStatus && Date.now() < endTime && !terminalStateReached) {
    ...
    const finalLogs = formatLogsForResponse(latestServerInfo?.logs ?? serverInfo.logs, linesToReturn);
    ...
    const responsePayload = {
        ...
        waitResult: achievedTargetStatus ? 'success' : (terminalStateReached ? 'wrong_state' : (timedOut ? 'timed_out' : 'unknown')), // Refined waitResult
        exitCode: serverInfo.exitCode, error: serverInfo.error, hint: finalHint,
        waitedSeconds: ((Date.now() - startTime) / 1000).toFixed(2),
        timeoutSetting: timeoutSeconds, logs: finalLogs, logLinesReturned: finalLogs.length,
    ...
    const result = {
        ...
        isError: (timedOut && targetStatusTyped !== 'stopped' && targetStatusTyped !== 'crashed' && targetStatusTyped !== 'error') || terminalStateReached || finalServerStatus === 'error' || finalServerStatus === 'crashed',
    };
    return result;
}
...
export function registerToolDefinitions(server: McpServer): void {
    const startDevServerSchema = z.object({
        ...
        cwd: z.string().optional().describe("The working directory to run the command from. If relative, it's resolved against the MCP workspace root or the current working directory of the MCP server process. Defaults to workspace root or process CWD."),
    ...
    server.tool(
        ...
        (params: CheckDevServerStatusParams) => handleToolCall(params.label, "check_dev_server_status", params, () =>
    ...
    server.tool(
        ...
        (params: StopDevServerParams) => handleToolCall(params.label, "stop_dev_server", params, () =>
    ...
    server.tool(
        ...
        (params: StopAllDevServersParams) => handleToolCall(null, "stop_all_dev_servers", params, () =>
    ...
    server.tool(
        ...
        (params: RestartDevServerParams) => handleToolCall(params.label, "restart_dev_server", params, () =>
    ...
    server.tool(
        ...
        (params: WaitForDevServerParams) => handleToolCall(params.label, "wait_for_dev_server", params, () =>
            _waitForDevServer(params.label, params.timeoutSeconds, params.checkIntervalSeconds, params.targetStatus, params.logLines)
        )
    );
} 